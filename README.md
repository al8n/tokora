...
- [Who Uses Tokora?](#who-uses-tokora)
- [License](#license)

## Overview

**Tokora** is a blazing fast parser combinator library for Rust that uniquely combines:
...
- **`Lexer<'inp>` Trait**
  ...
  Core trait for lexers that produce token streams. Implement this to use any lexer with Tokora.
...
Tokora's flexible `Emitter` system allows the same parser to adapt to different use cases by simply changing the error handling strategy.

#### Atomically Composable Trait Design

Tokora's emitter system uses **atomically composable traits** - small, focused traits that each handle a specific parsing scenario. Instead of one monolithic interface, error handling is broken down into atomic building blocks:
...
Tokora provides complete implementations that implement all atomic traits with consistent behavior:
...
**Key Design**: Change the `Emitter` type to switch between fail-fast runtime parsing and comprehensive compiler diagnostics - **same parser code, different behavior**. This makes Tokora suitable for both:
...
Tokora provides built-in parser combinators for resilient parsing that can continue after errors:
...
`tokora::guide` is a ten-chapter tutorial that builds a small language ("Calc") end to end — lexing,
...
The chapters live in `tokora/src/guide/*.md` and are the single source: `mod.rs` pulls each one in
...
(cd tokora && mdbook build)      # -> target/book/index.html
(cd tokora && mdbook serve)      # live preview on localhost:3000
```

The published guide is available at <https://al8n.github.io/tokora/>. GitHub Actions builds it for
...
`-D warnings`. A small mdbook preprocessor, `tokora/tools/mdbook_docs_links.py`, rewrites them to
...
Tokora's architecture follows a layered design:
...
Tokora uses a **parse-while-lexing** architecture where parsers consume tokens directly from the lexer as needed, without intermediate buffering:
...
Unlike traditional parser combinators that rely on implicit backtracking (trying alternatives until one succeeds), Tokora uses **explicit lookahead-based decisions**. This design choice provides:
...
// Tokora approach (explicit lookahead, no backtracking):
let parser = any()
...
Tokora's error handling system breaks down error scenarios into small, focused traits. Each trait handles one specific parsing situation (like `TooFewEmitter` for "too few elements" or `UnexpectedLeadingSeparatorEmitter` for leading separators).
...
Tokora's architecture decouples parsing logic from error handling strategy through the atomic `Emitter` trait system. This means:
