use crate::{
  ...
  emitter::{DelimiterEmitter, TooFewEmitter},
  error::syntax::TooFew,
};
...
impl<'inp, L, P, Open, Close, O, Condition, Container, Ctx, Delim, W, Lang: ?Sized>
  ...
  for Collect<
    DelimitedBy<
      AtLeast<Repeated<P, Condition, O, W, L, Ctx, Lang>>,
      Open,
      Close,
      Delim,
      O,
...
where
  ...
  W: Window,
  Ctx::Emitter: DelimiterEmitter<'inp, Delim, L, Lang> + TooFewEmitter<'inp, O, L, Lang>,
