...
impl Limitation {
      ...
      + TooManyEmitter<'inp, O, L, Lang>,
  {
    ...
    if num_elems > self.maximum {
      inp
        .emitter()
        .emit_too_many(TooMany::of(full_span.clone(), num_elems, self.maximum))?;
    }
    Ok(full_span)
  }
}
...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    &'c mut Bounded<SeparatedBy<&'c mut F, &'c mut SepClassifier, Condition, O, W, L, Ctx, Lang>>,
    &'c mut Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Wrapper<
    Collect<
      Bounded<SeparatedBy<&'c mut F, &'c mut SepClassifier, &'c mut Condition, O, W, L, Ctx, Lang>>,
      &'c mut Container,
      Ctx,
      Lang,
    >,
