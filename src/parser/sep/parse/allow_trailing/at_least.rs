use crate::{
  emitter::{TooFewEmitter, UnexpectedLeadingSeparatorEmitter},
  error::token::UnexpectedLeadingOf,
};

use super::*;

impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  EndStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for AllowTrailing<Minimum>
where
  L: Lexer<'inp>,
...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  ContinueStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for AllowTrailing<Minimum>
where
  L: Lexer<'inp>,
  Ctx: ParseContext<'inp, L, Lang>,
  Ctx::Emitter: SeparatedEmitter<'inp, O, Sep, L, Lang>,
...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  SeparatorStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for AllowTrailing<Minimum>
where
  L: Lexer<'inp>,
  Ctx: ParseContext<'inp, L, Lang>,
  Ctx::Emitter: SeparatedEmitter<'inp, O, Sep, L, Lang>
...
impl<'inp, L, F, SepClassifier, O, Container, Ctx, Lang: ?Sized>
  ...
  for Collect<
    AllowTrailing<AtLeast<Separated<F, SepClassifier, O, L, Ctx, Lang>>>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, SepClassifier, O, Container, Ctx, Lang: ?Sized>
  ...
  for Collect<
    &'c mut AllowTrailing<AtLeast<Separated<&'c mut F, &'c mut SepClassifier, O, L, Ctx, Lang>>>,
    &'c mut Container,
    Ctx,
    Lang,
  >
  ...
  Container: ContainerT<O> + SeparatorHandler<'inp, L>,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = AllowTrailing::new(AtLeast::new(
      Separated {
        f: &mut **f,
        sep: &mut **sep,
        _m: PhantomData,
...
impl<'inp, 'c, L, F, SepClassifier, O, Container, Ctx, Lang: ?Sized>
  ...
  for Wrapper<
    Collect<
      AllowTrailing<AtLeast<Separated<&'c mut F, &'c mut SepClassifier, O, L, Ctx, Lang>>>,
      &'c mut Container,
      Ctx,
      Lang,
    >,
