...
impl<P> RequireTrailing<P> {
  ...
  where
    Self: Apply<RequireTrailing<AtMost<P>>, Options = Maximum>,
  {
  ...
  where
    Self: Apply<RequireTrailing<AtLeast<P>>, Options = Minimum>,
  {
  ...
  where
    Self: Apply<RequireTrailing<Bounded<P>>, Options = With<Minimum, Maximum>>,
  {
