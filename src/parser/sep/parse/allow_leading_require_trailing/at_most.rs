...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  EndStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for AllowLeading<RequireTrailing<Maximum>>
where
  L: Lexer<'inp>,
...
impl<'inp, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    AllowLeading<
      RequireTrailing<AtMost<Separated<F, SepClassifier, Condition, O, W, L, Ctx, Lang>>>,
    >,
    Container,
    Ctx,
    Lang,
  ...
  W: Window,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = AllowLeading::new(RequireTrailing::new(AtMost::new(
      Separated {
        f: &mut **f,
        sep: &mut **sep,
  ...
  W: Window,
{
  ...
  fn parse_input(
    ...
    let limitation = AllowLeading::new(RequireTrailing::new(parser.parser.parser.maximum()));
