use crate::{
  emitter::{MissingLeadingSeparatorEmitter, TooManyEmitter, UnexpectedTrailingSeparatorEmitter},
...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  EndStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for RequireLeading<Maximum>
where
  L: Lexer<'inp>,
...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  ContinueStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for RequireLeading<Maximum>
where
  L: Lexer<'inp>,
  Ctx: ParseContext<'inp, L, Lang>,
  Ctx::Emitter:
...
impl<'inp, 'closure, Sep, O, L, Ctx, Lang: ?Sized>
  SeparatorStateHandler<'inp, 'closure, Sep, O, L, Ctx, Lang> for RequireLeading<Maximum>
where
  L: Lexer<'inp>,
  Ctx: ParseContext<'inp, L, Lang>,
  Ctx::Emitter: SeparatedEmitter<'inp, O, Sep, L, Lang>
...
impl<'inp, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    RequireLeading<AtMost<Separated<F, SepClassifier, Condition, O, W, L, Ctx, Lang>>>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    &'c mut RequireLeading<
      AtMost<Separated<&'c mut F, &'c mut SepClassifier, Condition, O, W, L, Ctx, Lang>>,
    >,
    &'c mut Container,
    Ctx,
    Lang,
  ...
  W: Window,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = RequireLeading::new(AtMost::new(
      Separated {
        f: &mut **f,
        sep: &mut **sep,
...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Wrapper<
    Collect<
      RequireLeading<
        AtMost<Separated<&'c mut F, &'c mut SepClassifier, &'c mut Condition, O, W, L, Ctx, Lang>>,
      >,
      &'c mut Container,
      Ctx,
      Lang,
  ...
  W: Window,
{
  ...
  fn parse_input(
    ...
    let limitation = RequireLeading::new(parser.parser.maximum());
