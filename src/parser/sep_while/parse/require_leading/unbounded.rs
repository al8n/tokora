use crate::{
  emitter::{MissingLeadingSeparatorEmitter, UnexpectedTrailingSeparatorEmitter},
...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    &'c mut RequireLeading<SeparatedWhile<F, SepClassifier, Condition, O, W, L, Ctx, Lang>>,
    &'c mut Container,
    Ctx,
    Lang,
  >
  ...
  W: Window,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = RequireLeading::new(SeparatedWhile {
      f: &mut *f,
      sep: &mut *sep,
