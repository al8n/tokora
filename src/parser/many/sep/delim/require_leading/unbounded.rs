...
impl<'inp, L, F, SepClassifier, O, Delim, Container, Ctx, Lang: ?Sized>
  ...
  for Collect<
    DelimitedBy<RequireLeading<Separated<F, SepClassifier, O, L, Ctx, Lang>>, Delim>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, SepClassifier, O, Delim, Container, Ctx, Lang: ?Sized>
  ...
  for Collect<
    &'c mut DelimitedBy<
      RequireLeading<Separated<&'c mut F, SepClassifier, O, L, Ctx, Lang>>,
      Delim,
    >,
    &'c mut Container,
    Ctx,
  ...
  Delim: Delimiter<'inp, L, Lang>,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = DelimitedBy::<_, Delim>::new_in(RequireLeading::new(Separated::new(&mut **f)));
...
impl<'inp, 'c, L, F, SepClassifier, O, Delim, Container, Ctx, Lang: ?Sized>
  ...
  for Wrapper<
    Collect<
      DelimitedBy<RequireLeading<Separated<&'c mut F, SepClassifier, O, L, Ctx, Lang>>, Delim>,
      &'c mut Container,
      Ctx,
      Lang,
    >,
  ...
  Delim: Delimiter<'inp, L, Lang>,
{
  ...
  fn parse_input(
    ...
    const UNBOUNDED: &RequireLeading<Unbounded> = &RequireLeading::new(Unbounded);
    ...
    let DelimitedBy {
      ...
      ..
    } = parser.map_parser_mut(|p| p.map_parser_mut(|p| p.as_mut()));
