...
impl<'inp, 'c, L, F, SepClassifier, Condition, O, Delim, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    &'c mut DelimitedBy<
      RequireTrailing<SeparatedWhile<&'c mut F, SepClassifier, Condition, O, W, L, Ctx, Lang>>,
      Delim,
    >,
    &'c mut Container,
    Ctx,
  ...
  Delim: Delimiter<'inp, L, Lang>,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = DelimitedBy::<_, Delim>::new_in(RequireTrailing::new(SeparatedWhile::new::<
      SepClassifier,
    >(
      &mut **f, &mut *condition
    )));
