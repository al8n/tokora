...
impl<'inp, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    AllowLeading<RequireTrailing<SeparatedWhile<F, SepClassifier, Condition, O, W, L, Ctx, Lang>>>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, L, F, SepClassifier, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for With<
    Collect<
      AllowLeading<
        RequireTrailing<SeparatedWhile<F, SepClassifier, Condition, O, W, L, Ctx, Lang>>,
      >,
      Container,
      Ctx,
      Lang,
  ...
  W: Window,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = AllowLeading::new(RequireTrailing::new(SeparatedWhile::new::<SepClassifier>(
      &mut *f,
      &mut *condition,
    )));
