...
// ── Str-shaped source (compact::Utf8Bytes) ─────────────────────────
//
// Mirrors the `Utf8Bytes` (shared) matrix above: same char-boundary
// semantics, just backed by the compact storage strategy.

#[test]
fn compact_utf8_is_empty_on_empty() {
  let src = compact::Utf8Bytes::new();
  assert!(Source::is_empty(&src));
}

#[test]
fn compact_utf8_is_empty_on_non_empty() {
  let src = compact::Utf8Bytes::from_static("abc");
  assert!(!Source::is_empty(&src));
}

#[test]
fn compact_utf8_len() {
  let src = compact::Utf8Bytes::from_static("hello");
  assert_eq!(Source::len(&src), 5);
}

#[test]
fn compact_utf8_len_empty() {
  let src = compact::Utf8Bytes::new();
  assert_eq!(Source::len(&src), 0);
}

#[test]
fn compact_utf8_len_multibyte() {
  // Each emoji is 4 bytes in UTF-8.
  let src = compact::Utf8Bytes::from_static("\u{1F600}");
  assert_eq!(Source::len(&src), 4);
}

#[test]
fn compact_utf8_slice_full_range() {
  let src = compact::Utf8Bytes::from_static("abcde");
  let result = Source::slice(&src, 0..5);
  assert_eq!(result.as_deref(), Some("abcde"));
}

#[test]
fn compact_utf8_slice_partial() {
  let src = compact::Utf8Bytes::from_static("abcde");
  let result = Source::slice(&src, 1..3);
  assert_eq!(result.as_deref(), Some("bc"));
}
...
#[test]
fn compact_utf8_slice_out_of_bounds() {
  let src = compact::Utf8Bytes::from_static("abc");
  let result = Source::slice(&src, 0..10);
  assert!(result.is_none());
}
...
#[test]
fn compact_utf8_slice_on_non_boundary_returns_none() {
  // 2-byte char: the second byte is not a valid boundary.
  let src = compact::Utf8Bytes::from_static("\u{00E9}abc"); // e-acute (2 bytes) + abc
  let result = Source::slice(&src, 0..1);
  assert!(result.is_none());
}

#[test]
fn compact_utf8_is_boundary_at_char_boundaries() {
  let src = compact::Utf8Bytes::from_static("\u{00E9}a"); // 2-byte char + 1-byte char
  assert!(Source::is_boundary(&src, 0));
  assert!(!Source::is_boundary(&src, 1)); // middle of 2-byte char
  assert!(Source::is_boundary(&src, 2)); // start of 'a'
  assert!(Source::is_boundary(&src, 3)); // end
...
#[test]
fn compact_utf8_is_boundary_at_end() {
  let src = compact::Utf8Bytes::from_static("abc");
  assert!(Source::is_boundary(&src, 3));
}

#[test]
fn compact_utf8_is_boundary_beyond_len() {
  let src = compact::Utf8Bytes::from_static("abc");
  assert!(!Source::is_boundary(&src, 4));
}

#[test]
fn compact_utf8_find_boundary_rounds_down_multibyte() {
  // "é" is a single 2-byte code point occupying 0..2.
  let src = compact::Utf8Bytes::from_static("\u{00E9}");
  assert_eq!(Source::find_boundary(&src, 1), 0);
}

#[test]
fn compact_utf8_find_boundary_rounds_down_after_ascii() {
  // 'a' at 0, "é" at 1..3.
  let src = compact::Utf8Bytes::from_static("a\u{00E9}");
  assert_eq!(Source::find_boundary(&src, 2), 1);
}

#[test]
fn compact_utf8_find_boundary_passes_through_boundaries() {
  let src = compact::Utf8Bytes::from_static("a\u{00E9}"); // boundaries at 0, 1, and 3 (== len)
  assert_eq!(Source::find_boundary(&src, 0), 0);
  assert_eq!(Source::find_boundary(&src, 1), 1);
  assert_eq!(Source::find_boundary(&src, 3), 3);
}
...
#[test]
fn compact_utf8_find_boundary_at_and_beyond_len() {
  // index >= len is returned unchanged, symmetric with the byte sources.
  let src = compact::Utf8Bytes::from_static("a\u{00E9}"); // len 3
  assert_eq!(Source::find_boundary(&src, 3), 3);
  assert_eq!(Source::find_boundary(&src, 10), 10);
}
