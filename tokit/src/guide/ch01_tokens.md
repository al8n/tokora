...
A tokit token is two types working together, connected by the [`Token`](crate::Token) trait:
...
Any type implementing [`Lexer`](crate::Lexer) can drive tokit's parsers. Calc does not
...
// The bridge: tokit's `Token` trait names the kind and the lexer error type.
