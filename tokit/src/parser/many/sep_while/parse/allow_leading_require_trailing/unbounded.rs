...
impl<'inp, L, F, Sep, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    AllowLeading<RequireTrailing<SeparatedWhile<F, Sep, Condition, O, W, L, Ctx, Lang>>>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, Sep, Condition, O, Container, Ctx, Lang: ?Sized, W>
  ...
  for Collect<
    &'c mut AllowLeading<RequireTrailing<SeparatedWhile<F, Sep, Condition, O, W, L, Ctx, Lang>>>,
    &'c mut Container,
    Ctx,
    Lang,
  >
  ...
  W: Window,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = AllowLeading::new(RequireTrailing::new(SeparatedWhile::new::<Sep>(
      &mut *f,
      &mut *condition,
    )));
