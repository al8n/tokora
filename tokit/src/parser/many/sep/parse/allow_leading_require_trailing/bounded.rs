...
impl<'inp, L, F, Sep, O, Container, Ctx, Lang: ?Sized> ParseInput<'inp, L, Container, Ctx, Lang>
  for Collect<
    AllowLeading<RequireTrailing<Bounded<Separated<F, Sep, O, L, Ctx, Lang>>>>,
    Container,
    Ctx,
    Lang,
  >
...
impl<'inp, 'c, L, F, Sep, O, Container, Ctx, Lang: ?Sized> ParseInput<'inp, L, L::Span, Ctx, Lang>
  for Collect<
    &'c mut AllowLeading<RequireTrailing<Bounded<Separated<&'c mut F, Sep, O, L, Ctx, Lang>>>>,
    &'c mut Container,
    Ctx,
    Lang,
  >
  ...
  Container: ContainerT<O> + SeparatorHandler<'inp, L>,
{
    ...
    Ctx: ParseContext<'inp, L, Lang>,
  {
    ...
    let parser = AllowLeading::new(RequireTrailing::new(Bounded::new(
...
impl<'inp, 'c, L, F, Sep, O, Container, Ctx, Lang: ?Sized> ParseInput<'inp, L, L::Span, Ctx, Lang>
  for Wrapper<
    Collect<
      AllowLeading<RequireTrailing<Bounded<Separated<&'c mut F, Sep, O, L, Ctx, Lang>>>>,
      &'c mut Container,
      Ctx,
      Lang,
    >,
  ...
  Container: ContainerT<O> + SeparatorHandler<'inp, L>,
{
  ...
  fn parse_input(
    ...
    let limitation = AllowLeading::new(RequireTrailing::new(parser.parser.parser.to_with()));
