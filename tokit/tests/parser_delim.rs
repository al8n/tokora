...
// ── Custom emitter for require_* variants ─────────────────────────────────────
//
// `Fatal<DelimError>` does NOT implement `MissingTrailingSeparatorEmitter` or
// `MissingLeadingSeparatorEmitter`, so `require_*` variants need a custom emitter.
...
impl<'inp> Emitter<'inp, TestLexer<'inp>> for FullEmitter {
  ...
  fn emit_error(
    &mut self,
    err: Spanned<DelimError, <TestLexer<'inp> as Lexer<'inp>>::Span>,
  ) -> Result<(), DelimError>
  where
    TestLexer<'inp>: Lexer<'inp>,
  {
