//! A simple arithmetic expression evaluator using [`InputRef::pratt`](tokit::InputRef::pratt).
//!
//! Demonstrates the token-level Pratt API where the token type implements
//! [`PrattToken`](tokit::token::PrattToken) to classify itself as an operand, prefix,
//! or infix operator. Fold functions receive raw [`Spanned`](tokit::span::Spanned) tokens
...
type CalcLexer<'a> = tokit::lexer::LogosLexer<'a, Token>;
...
impl<'inp> From<tokit::error::token::UnexpectedTokenOf<'inp, CalcLexer<'inp>>> for CalcError {
  fn from(_: tokit::error::token::UnexpectedTokenOf<'inp, CalcLexer<'inp>>) -> Self {
    CalcError::UnexpectedToken
  }
}

impl From<tokit::error::UnexpectedEot> for CalcError {
  fn from(_: tokit::error::UnexpectedEot) -> Self {
