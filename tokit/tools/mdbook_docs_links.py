...
To refresh after an API move: RUSTDOCFLAGS= cargo doc -p tokit --all-features --no-deps, then
compare the hrefs under target/doc/tokit/guide/*/index.html with the values below.
"""
...
DOCS_RS = "https://docs.rs/tokit/latest/tokit/"

# rustdoc-verified: crate:: path -> page (+anchor) under https://docs.rs/tokit/latest/tokit/.
...
def main() -> int:
  ...
  if errors:
    print(
      ...
      "render as dead links in the book. Add them to tokit/tools/mdbook_docs_links.py:",
      file=sys.stderr,
    )
